...
int drawCard(void){
	return rand()%13+1;//Write the function to random the number from 1 to 13 and return that random number.
	//srand() is used in main(). Do not use srand() here.
}

int calScore(int x,int y,int z){
	return (cardScores[x]+cardScores[y]+cardScores[z])%10;//Write the function to calculate the score with is the rightmost digit of summation of the scores from three cards.
	//Return the score. 
}
